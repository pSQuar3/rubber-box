...
void solve()
{
    ...
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<m;j++)
        {
            ...
            {
                if(sm-a[i][j]-a[i][j-1]+(-1)*a[i][j]+(-1)*a[i][j-1] > sm)
                {
                    sm = sm-a[i][j]-a[i][j-1]+(-1)*a[i][j]+(-1)*a[i][j-1];
//                    a[i][j] = (-1)*a[i][j];
//                    a[i][j-1] = (-1)*a[i][j-1];
                }
                if(sm-a[i][j]-a[i-1][j]+(-1)*a[i][j]+(-1)*a[i-1][j] > sm)
                {
                    sm = sm-a[i][j]-a[i-1][j]+(-1)*a[i][j]+(-1)*a[i-1][j];
//                    a[i][j] = (-1)*a[i][j];
//                    a[i-1][j] = (-1)*a[i-1][j];
                }
