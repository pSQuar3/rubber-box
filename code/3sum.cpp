...
void solve()
{
    // statement: Given an integer array a[] and an integer k, find any 2 numbers in the array so that their sum is k
    int n,k;
    cin >> n >> k;
    for(int i=0;i<n;i++)
        cin >> a[i];
    ...
    for(int i=0;i<n-2;i++)
    {
        ...
        if(i < st && st < et && a[i] + a[st] + a[et] == k)
            cout << a[i] << " " << a[st] << " " << a[et] << endl;
    }
}
int main()
{
