...
int main()
{
    ...
    map<int,int>::iterator ml = boys.begin();
    map<int,int>::iterator fml = girls.begin();
    int c=0;
    while((ml != boys.end() && fml != girls.end()))
    {
        if(ml->first - fml->first > 1)
            fml++;
        if(fml->first - ml->first > 1)
            ml++;
        if(abs(fml->first - ml->first) <= 1)
        {
            if(fml->second > 0 && ml->second > 0)
            {
                fml->second = fml->second - 1;
                ml->second = ml->second - 1;
                c++;
            }
            if(fml->second == 0)
                fml++;
